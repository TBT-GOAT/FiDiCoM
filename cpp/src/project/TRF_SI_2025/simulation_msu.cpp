                           ...
                           size_t seed) 
{
    ...
    rDn_2_WTSN rdn_2_wtsn(25000, domain_polygon);
    ...
    // std::ofstream g {"/home/workspace/matrix_distance.txt"};
    // for (const auto& dv : distance_matrix) {
    ...
    // for (const auto& [type, population] : population_map) {
    //     std::cout << "Population of " << type << ": " << population << std::endl;
    // }
    // std::cout << "Total population: " << total_population << std::endl;
    // std::cout << "Busstop population: " << busstop_population << std::endl;
    // std::cout << "Cafeteria population: " << cafeteria_population << std::endl;
    // std::cout << "Entrance population: " << entrance_population << std::endl;
    ...
    // std::ofstream fg {"/home/workspace/matrix_population_outer.txt"};
    // for (const auto& pov : pop_outer_matrix) {
    //     for (const double po : pov) {
    //         fg << std::scientific 
    //             << std::setprecision(std::numeric_limits<double>::max_digits10) 
    //             << po << ",";
    //     }
    ...
    // std::ofstream fh {"/home/workspace/matrix_utility.txt"};
    // for (const auto& uv : utility_matrix) {
    //     for (const double u : uv) {
    //         fh << std::scientific 
    //             << std::setprecision(std::numeric_limits<double>::max_digits10) 
    //             << u << ",";
    //     }
    ...
    // for (size_t i = 0; i < simulator.get_demand_nodes().size(); ++i) {
    //     rDn_2_WTSN::vertex_descriptor demand_node = simulator.get_demand_nodes().at(i);
    //     std::shared_ptr<Node_2> node_ptr = (*simulator.get_net_ptr())[demand_node];
    ...
    // std::ofstream h {"/home/workspace/commands2.txt"};
    // Net_2::edge_iterator edge_it, edge_end;
    // for (boost::tie(edge_it, edge_end) = boost::edges(wtsn); edge_it != edge_end; ++edge_it) {
    //     std::shared_ptr<Node_2> s_ptr = wtsn[boost::source(*edge_it, wtsn)];
    //     std::shared_ptr<Node_2> t_ptr = wtsn[boost::target(*edge_it, wtsn)];
    //     h << std::scientific 
    //         << std::setprecision(std::numeric_limits<double>::max_digits10)
    //         << "line" << std::endl 
    //         << s_ptr->x() << "," << s_ptr->y() << ",0.0" << std::endl
...
int run_simulation_msu(double init_w, size_t damp_count, size_t amp_count, double b, size_t s) {
    return execute_simulation_msu(init_w, damp_count, amp_count, b, s);
}
...
//     if (args.size() < 7) {
//         throw std::runtime_error("Insufficient arguments for MSU simulation.");
//     }
...
//     const double beta = std::stod(beta_str);
...
//     // ファイルの読み込み
//     std::string ifile_path = "/home/workspace/data/simulation_MSU/setting.json";
//     std::ifstream input_file(ifile_path);
//     if (!input_file) {
//         std::cerr << "Error opening file: " << ifile_path << std::endl;
//         return EXIT_FAILURE;
...
//     rDn_2_WTSN rdn_2_wtsn(50000, domain_polygon);
//     rdn_2_wtsn.initialize();
...
//     for (size_t i = 0; i < wtsn_demand_nodes.size(); ++i) {
//         for (size_t j = 0; j < wtsn_demand_nodes.size(); ++j) {
//             std::cout << "calculate path from " << i << " to " << j << std::endl;
//             std::cout << "from " << wtsn[wtsn_demand_nodes.at(i)]->x() << "," 
//                         << wtsn[wtsn_demand_nodes.at(i)]->y() << " to "
//                         << wtsn[wtsn_demand_nodes.at(j)]->x() << "," 
//                         << wtsn[wtsn_demand_nodes.at(j)]->y() << std::endl;
//             std::deque<std::pair<Net_2::vertex_descriptor, double>> path = wtsn.calculate_shortest_path(wtsn_demand_nodes.at(i), 
//                                                                                                         wtsn_demand_nodes.at(j),
//                                                                                                         Net_2::MODE_ROUTE);
//             std::cout << "path size: " << path.size() << std::endl;
//             std::cout << "calculate path length" << std::endl;
//             D_distance_matrix.at(i).at(j) = wtsn.calculate_path_length(path, 
//                                                                        Net_2::MODE_ROUTE);
//         }
//     }
...
//     std::ofstream fd {"/home/workspace/matrix_detour.txt"};
//     for (const auto& dv : detour_matrix) {
//         for (const double d : dv) {
//             fd << std::scientific 
//                 << std::setprecision(std::numeric_limits<double>::max_digits10)
